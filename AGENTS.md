...
- **Database**: `packages/database/` — Drizzle ORM schema + client (`@ekajaya/database`)
...
bun run build --filter=@ekajaya/api  # Build api only
```
...
- **Local commands** (run from repo root, `apps/api/.dev.vars` must exist):
  - `bun run db:generate --filter=@ekajaya/api` — diff schema against remote D1, create migration SQL.
  - `bun run db:migrate --filter=@ekajaya/api` — apply pending migrations to production D1.
  - `bun run db:push --filter=@ekajaya/api` — push schema directly (bypass migrations, dev only).
  - `bun run db:studio --filter=@ekajaya/api` — open Drizzle Studio (visual DB browser).
...
│   │   ├── public/
│   │   └── astro.config.mjs
│   └── api/                # @ekajaya/api — TanStack Start → Cloudflare Workers
│       ├── src/
...
│   ├── database/             # @ekajaya/database — Drizzle ORM schema + client
│   ├── hooks/                # @ekajaya/hooks — useMediaQuery, useContainerSize
│   ├── http/                 # @ekajaya/http — ky API client
│   ├── schema/               # @ekajaya/schema — Zod validation schemas
